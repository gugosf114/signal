...
export default function LoadingTheater({ cardName, game, onCancel, startedAt = null, complete = false }) {
  ...
  return (
    <div ref={rootRef} className="lt-canvas" data-jp={phase.jp || undefined}>
      ...
      {/* Way out. Tapping the wordmark has always aborted an in-flight scan,
          but nothing on screen said so — a scan started by mistake left the
          user watching a minute of theater with no visible way to stop it.
          Wired to the same abort path, just where you'd look for it. */}
      {onCancel && (
        <div className="lt-cancel-row">
          <button type="button" className="lt-cancel" onClick={onCancel}>
